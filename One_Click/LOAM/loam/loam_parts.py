...
class Dense(nn.Module):
    def __init__(self, in_scales, out_scales):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(in_scales, out_scales),
            nn.ReLU(inplace=True)
        )
...
class mf_layer(nn.Module):
    ...
    def __init__(self, channel, groups=64):
        super().__init__()
        self.groups = groups
        #self.avg_pool1d = nn.AdaptiveAvgPool1d(groups)
    ...
    def forward(self, x, zc0, zm0):
        ...
        # zm: (b, 1, 1, 16)
        zm = zm.unsqueeze(1).unsqueeze(1)
        zms = [zm[i].repeat(self.groups, 1, 1, 1) for i in range(b)]
        # zm: (b*groups, 1, 1, 16)
        zm = torch.cat(zms, dim=0)
        zm = self.sweight * zm + self.sbias
        zmt = x_1 * self.sigmoid(zm)

        out = torch.cat([zct, zmt], dim=1)
